...
const securityMiddleware = async (req, res, next) => {
  try {
    ...
    const rule = slidingWindow
      ...
      : null;
    const client =
      aj && typeof aj.withRule === 'function' && rule ? aj.withRule(rule) : aj;

    const decision =
      client && typeof client.protect === 'function'
    ...
    const denied =
      typeof decision.isDenied === 'function' ? decision.isDenied() : false;
    ...
    if (
      ...
      logger.warn(
        `Bot detected: ${typeof reason.getDetails === 'function' ? reason.getDetails() : 'n/a'}`
      );
      return res
        .status(403)
        .json({ message: 'Access denied. Bot activity detected.' });
    }

    if (
      denied &&
      reason &&
      typeof reason.isShield === 'function' &&
      reason.isShield()
    ) {
      logger.warn(
        `Request denied by Arcjet Shield: ${typeof reason.getDetails === 'function' ? reason.getDetails() : 'n/a'}`
      );
      return res
        .status(403)
    ...
    if (
      ...
      logger.warn(
        `Rate limit exceeded for role ${role}: ${typeof reason.getDetails === 'function' ? reason.getDetails() : 'n/a'}`
      );
